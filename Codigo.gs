...
// 2. INICIALIZAR O CREAR TODAS LAS PESTAÑAS AUTOMÁTICAMENTE
function inicializarPestanas() {
  crearEstructuraInicialSiEsNecesario();
}

function crearEstructuraInicialSiEsNecesario() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  
  // 1. Pestaña Solicitudes Clientes
  var sheetSol = ss.getSheetByName("Solicitudes Clientes") || ss.getSheetByName("Solicitudes") || ss.getSheetByName("Clientes");
  if (!sheetSol) {
    sheetSol = ss.insertSheet("Solicitudes Clientes");
  }
  if (sheetSol.getLastRow() === 0) {
    var encabSol = [
    ...
    sheetSol.getRange(1, 1, 1, encabSol.length)
      .setFontWeight("bold")
      .setBackground("#25815F")
      .setFontColor("#FFFFFF")
      .setHorizontalAlignment("center");
    sheetSol.setFrozenRows(1);
    for (var c1 = 1; c1 <= encabSol.length; c1++) sheetSol.autoResizeColumn(c1);
  }

  // 2. Pestaña Lista de Espera
  var sheetWait = ss.getSheetByName("Lista de Espera");
  if (!sheetWait) {
    sheetWait = ss.insertSheet("Lista de Espera");
  }
  if (sheetWait.getLastRow() === 0) {
    var encabWait = [
      "Fecha", "Nombre", "Teléfono", "Email", "Turno Deseado", "Plan Deseado",
      "Sector / CP", "Calle y Nº", "Piso / Puerta", "Estado", "Observaciones"
    ];
    sheetWait.appendRow(encabWait);
    sheetWait.getRange(1, 1, 1, encabWait.length)
      .setFontWeight("bold")
      .setBackground("#D97706")
      .setFontColor("#FFFFFF")
      .setHorizontalAlignment("center");
    sheetWait.setFrozenRows(1);
    for (var c2 = 1; c2 <= encabWait.length; c2++) sheetWait.autoResizeColumn(c2);
  }

  // 3. Pestaña Candidaturas (Colaboradores)
  var sheetCand = ss.getSheetByName("Candidaturas") || ss.getSheetByName("Colaboradores");
  if (!sheetCand) {
    sheetCand = ss.insertSheet("Candidaturas");
  }
  if (sheetCand.getLastRow() === 0) {
    var encabCand = [
    ...
    sheetCand.appendRow(encabCand);
    sheetCand.getRange(1, 1, 1, encabCand.length)
      .setFontWeight("bold")
      .setBackground("#1E6B4E")
      .setFontColor("#FFFFFF")
      .setHorizontalAlignment("center");
    sheetCand.setFrozenRows(1);
    for (var c3 = 1; c3 <= encabCand.length; c3++) sheetCand.autoResizeColumn(c3);
  }

  // 4. Pestaña Hoja de Ruta (Reparto y Google Maps)
  var sheetRuta = ss.getSheetByName("Hoja de Ruta");
  if (!sheetRuta) {
    sheetRuta = ss.insertSheet("Hoja de Ruta");
  }
  if (sheetRuta.getLastRow() === 0) {
    // Controles superiores en fila 1
    sheetRuta.getRange("A1").setValue("Día:").setFontWeight("bold").setHorizontalAlignment("right");
    sheetRuta.getRange("B1").setValue("Hoy").setFontWeight("bold").setHorizontalAlignment("center").setBackground("#E8F5EF");
    sheetRuta.getRange("C1").setValue("Turno:").setFontWeight("bold").setHorizontalAlignment("right");
    sheetRuta.getRange("D1").setValue("Todos").setFontWeight("bold").setHorizontalAlignment("center").setBackground("#E8F5EF");
    sheetRuta.getRange("F1").setValue("🗺️ ABRIR RUTA EN GOOGLE MAPS").setFontWeight("bold").setHorizontalAlignment("center").setBackground("#25815F").setFontColor("#FFFFFF");

    // Encabezados de columnas en fila 2
    var encabRuta = [
      "Orden", "Calle y Número", "Portal / Piso / Puerta", "Cliente", "Teléfono", "Horario / Turno", "Notas de Recogida"
    ];
    sheetRuta.getRange(2, 1, 1, encabRuta.length).setValues([encabRuta]);
    sheetRuta.getRange(2, 1, 1, encabRuta.length)
      .setFontWeight("bold")
      .setBackground("#13402E")
      .setFontColor("#FFFFFF")
      .setHorizontalAlignment("center");
    sheetRuta.setFrozenRows(2);
    for (var c4 = 1; c4 <= encabRuta.length; c4++) sheetRuta.autoResizeColumn(c4);
  }

  // 5. Pestaña Peticiones Nueva Zona (Expansión)
  var sheetZonas = ss.getSheetByName("Peticiones Nueva Zona") || ss.getSheetByName("Expansión Zonas");
  if (!sheetZonas) {
    sheetZonas = ss.insertSheet("Peticiones Nueva Zona");
  }
  if (sheetZonas.getLastRow() === 0) {
    var encabZonas = [
      "Fecha", "Nombre", "Teléfono", "Email", "Código Postal", "Municipio / Barrio", 
      "Tipo Vivienda / Plan Deseado", "Observaciones", "Estado"
    ];
    sheetZonas.appendRow(encabZonas);
    sheetZonas.getRange(1, 1, 1, encabZonas.length)
      .setFontWeight("bold")
      .setBackground("#1E5E44")
      .setFontColor("#FFFFFF")
      .setHorizontalAlignment("center");
    sheetZonas.setFrozenRows(1);
    for (var c5 = 1; c5 <= encabZonas.length; c5++) sheetZonas.autoResizeColumn(c5);
  }

  SpreadsheetApp.getUi().alert("✅ ¡Las 5 Pestañas de TeLaTiro están creadas y verificadas!\n\n1. 🟢 Solicitudes (Clientes activos)\n2. 🟠 Lista de Espera (Turnos completos)\n3. 💼 Candidaturas (Colaboradores y Empresas)\n4. 🗺️ Hoja de Ruta (Orden de paradas y Google Maps)\n5. 📍 Peticiones Nueva Zona (Expansión y nuevas áreas)");
}
...
// CONFIGURACIÓN OFICIAL DE CUPOS DIARIOS POR TURNO (30 Plazas Mañana / 30 Plazas Tarde = 60 Plazas)
// Distribución por Código Postal: 28523 (24) | 28522 (26) | 28521 (10) | 28524 (0 - Futura Expansión)
var MAX_SLOTS_MORNING = 30;   // Cupo máximo Turno Mañana (09:00 - 13:00 h)
var MAX_SLOTS_AFTERNOON = 30; // Cupo máximo Turno Tarde (16:00 - 20:00 h)
...
function doGet(e) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = ss.getSheetByName("Solicitudes Clientes") || ss.getSheetByName("Solicitudes") || ss.getSheetByName("Clientes") || ss.getSheetByName("Hoja 1") || ss.getSheets()[0];
    
    var bookedMorning = 0;
    var bookedAfternoon = 0;

    var zoneBooked = {
      '28523': { morning: 0, afternoon: 0 },
      '28522': { morning: 0, afternoon: 0 },
      '28521': { morning: 0, afternoon: 0 },
      '28524': { morning: 0, afternoon: 0 }
    };
    ...
    if (sheet && sheet.getLastRow() > 1) {
      ...
      for (var c = 0; c < headers.length; c++) {
        var h = headers[c];
        if (h.indexOf("turno") !== -1 || h.indexOf("franja") !== -1 || h.indexOf("horario") !== -1) {
          colTurno = c;
        }
        if (h.indexOf("estado") !== -1 || h.indexOf("situación") !== -1 || h.indexOf("situacion") !== -1) {
          colEstado = c;
        }
        if (h.indexOf("plan") !== -1 || h.indexOf("tipo") !== -1 || h.indexOf("servicio") !== -1) {
          colPlan = c;
        }
        if (h.indexOf("sector") !== -1 || h.indexOf("código postal") !== -1 || h.indexOf("codigo postal") !== -1 || h.indexOf("cp") !== -1) {
          colSector = c;
        }
        if (h.indexOf("calle") !== -1 || h.indexOf("dirección") !== -1 || h.indexOf("direccion") !== -1) {
          colCalle = c;
        }
      }
      ...
      if (colPlan === -1) colPlan = 4;
      if (colSector === -1) colSector = 6;
      if (colCalle === -1) colCalle = 7;
      ...
      for (var i = 1; i < data.length; i++) {
        ...
        var plan = (colPlan < row.length && row[colPlan] ? row[colPlan] : "").toString().toLowerCase();
        var sectorText = ((colSector < row.length && row[colSector] ? row[colSector] : "") + " " + (colCalle < row.length && row[colCalle] ? row[colCalle] : "")).toLowerCase();
        ...
        // Ignorar servicios puntuales: no son suscripciones mensuales recurrentes y NO restan plazas fijas
        if (plan.indexOf("puntual") !== -1 || plan.indexOf("suelto") !== -1 || plan.indexOf("1 día") !== -1 || plan.indexOf("1 dia") !== -1) {
          continue;
        }

        var isMorning = (turno.indexOf("mañana") !== -1 || turno.indexOf("09") !== -1);
        var isAfternoon = (turno.indexOf("tarde") !== -1 || turno.indexOf("16") !== -1);

        if (!isMorning && !isAfternoon) {
          isMorning = true; // Por defecto mañana si no se especifica
        }

        // Determinar código postal
        var zKey = '28523';
        if (sectorText.indexOf('28523') !== -1) zKey = '28523';
        else if (sectorText.indexOf('28522') !== -1) zKey = '28522';
        else if (sectorText.indexOf('28521') !== -1) zKey = '28521';
        else if (sectorText.indexOf('28524') !== -1 || sectorText.indexOf('28525') !== -1) zKey = '28524';

        if (isMorning) {
          bookedMorning++;
          if (zoneBooked[zKey]) zoneBooked[zKey].morning++;
        } else if (isAfternoon) {
          bookedAfternoon++;
          if (zoneBooked[zKey]) zoneBooked[zKey].afternoon++;
        }
      }
    }
    ...
    var zonesOutput = {
      '28523': {
        ...
        maxMorning: 12,
        maxAfternoon: 12,
        bookedMorning: zoneBooked['28523'].morning,
        bookedAfternoon: zoneBooked['28523'].afternoon,
        morning: Math.max(0, 12 - zoneBooked['28523'].morning),
        afternoon: Math.max(0, 12 - zoneBooked['28523'].afternoon),
        total: Math.max(0, 12 - zoneBooked['28523'].morning) + Math.max(0, 12 - zoneBooked['28523'].afternoon),
        isMorningFull: (12 - zoneBooked['28523'].morning) <= 0,
        isAfternoonFull: (12 - zoneBooked['28523'].afternoon) <= 0,
        isFuture: false
      },
      '28522': {
        ...
        maxMorning: 13,
        maxAfternoon: 13,
        bookedMorning: zoneBooked['28522'].morning,
        bookedAfternoon: zoneBooked['28522'].afternoon,
        morning: Math.max(0, 13 - zoneBooked['28522'].morning),
        afternoon: Math.max(0, 13 - zoneBooked['28522'].afternoon),
        total: Math.max(0, 13 - zoneBooked['28522'].morning) + Math.max(0, 13 - zoneBooked['28522'].afternoon),
        isMorningFull: (13 - zoneBooked['28522'].morning) <= 0,
        isAfternoonFull: (13 - zoneBooked['28522'].afternoon) <= 0,
        isFuture: false
      },
      '28521': {
        ...
        maxMorning: 5,
        maxAfternoon: 5,
        bookedMorning: zoneBooked['28521'].morning,
        bookedAfternoon: zoneBooked['28521'].afternoon,
        morning: Math.max(0, 5 - zoneBooked['28521'].morning),
        afternoon: Math.max(0, 5 - zoneBooked['28521'].afternoon),
        total: Math.max(0, 5 - zoneBooked['28521'].morning) + Math.max(0, 5 - zoneBooked['28521'].afternoon),
        isMorningFull: (5 - zoneBooked['28521'].morning) <= 0,
        isAfternoonFull: (5 - zoneBooked['28521'].afternoon) <= 0,
        isFuture: false
      },
      '28524': {
...
function doPost(e) {
  try {
    ...
    // ========================================================================
    // CASO D: PETICIÓN DE APERTURA DE NUEVA ZONA / EXPANSIÓN
    // ========================================================================
    if (data.formType === "peticion_zona" || data.formType === "expansion_zona" || data.formType === "nueva_zona" || data.action === "nueva_zona") {
      var sheetZonas = ss.getSheetByName("Peticiones Nueva Zona") || ss.getSheetByName("Expansión Zonas");
      if (!sheetZonas) {
        sheetZonas = ss.insertSheet("Peticiones Nueva Zona");
        var encabZonas = [
      ...
      sheetZonas.appendRow([
        fechaActual,
        data.name || "-",
        data.phone || "-",
        data.email || "-",
        data.zip || "-",
        data.municipality || data.zone || "-",
        data.plan || data.housingType || "-",
        data.notes || "Sin observaciones",
        "Interesado Registrado"
      ]);

      var cleanPhoneZona = (data.phone || "").toString().replace(/[^0-9]/g, "");
      var emailZona = (data.email || "").toString().trim();
      var nombreZona = (data.name || "Vecino interesado").toString().trim();
      var cpZona = (data.zip || "").toString().trim();
      var muniZona = (data.municipality || data.zone || "Nueva Zona").toString().trim();

      // Notificación a administradores
      var asuntoAdminZona = "📍 ¡Nueva Petición de Apertura de Zona! - " + (cpZona ? "CP " + cpZona + " " : "") + muniZona + " (" + nombreZona + ")";
      var htmlAdminZona = `
        <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; padding: 20px;">
          <div style="background-color: #25815F; padding: 15px; border-radius: 8px; color: white; text-align: center;">
            <h2 style="margin: 0; color: white;">📍 TeLaTiro • Solicitud de Apertura de Zona</h2>
          </div>
          
          <h3 style="color: #25815F; border-bottom: 2px solid #E8F5EF; padding-bottom: 5px; margin-top: 20px;">Datos de la Petición</h3>
          <p><strong>👤 Nombre:</strong> ${nombreZona}</p>
          <p><strong>📞 Teléfono:</strong> <a href="tel:${cleanPhoneZona}">${data.phone || "-"}</a></p>
          <p><strong>✉️ Email:</strong> <a href="mailto:${emailZona}">${emailZona || "-"}</a></p>
          <p><strong>📮 Código Postal:</strong> <span style="background: #E8F5EF; color: #13402E; padding: 3px 8px; border-radius: 4px; font-weight: bold;">${cpZona || "-"}</span></p>
          <p><strong>🏙️ Municipio / Barrio:</strong> ${muniZona}</p>
          <p><strong>🏠 Vivienda / Plan de Interés:</strong> ${data.plan || data.housingType || "-"}</p>
          <p><strong>📝 Comentarios / Dirección:</strong> ${data.notes || "Sin comentarios"}</p>
          
          <div style="margin-top: 25px; padding-top: 15px; border-top: 1px solid #eee; text-align: center; font-size: 12px; color: #888;">
            Gestión de Expansión TeLaTiro • Registro guardado en pestaña <strong>Peticiones Nueva Zona</strong>
          </div>
        </div>
      `;

      for (var dZ = 0; dZ < destinatariosAdmin.length; dZ++) {
        try {
          MailApp.sendEmail({
            to: destinatariosAdmin[dZ],
            subject: asuntoAdminZona,
            htmlBody: htmlAdminZona,
            name: "TeLaTiro Expansión"
          });
        } catch (eMailZ) {
          console.warn("No se pudo enviar email admin zona:", eMailZ);
        }
      }

      // Acuse de recibo al usuario
      if (emailZona && emailZona.indexOf("@") !== -1) {
        try {
          MailApp.sendEmail({
            to: emailZona,
            subject: "📍 Hemos recibido tu petición para " + (cpZona ? "CP " + cpZona + " (" + muniZona + ")" : muniZona) + " • TeLaTiro",
            htmlBody: `
              <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; border: 1px solid #e0e0e0; border-radius: 12px; overflow: hidden; padding: 20px;">
                <div style="background-color: #25815F; padding: 15px; border-radius: 8px; color: white; text-align: center;">
                  <h2 style="margin: 0; color: white;">¡Gracias por tu interés en TeLaTiro!</h2>
                </div>
                
                <p style="margin-top: 20px;">Hola <strong>${nombreZona}</strong>,</p>
                <p>Hemos registrado correctamente tu solicitud de servicio para <strong>${cpZona ? "CP " + cpZona + " - " : ""}${muniZona}</strong>.</p>
                <p>Estamos agrupando las peticiones de tu municipio y organizando las próximas rutas de apertura. <strong>Te avisaremos por WhatsApp o correo electrónico de forma prioritaria en cuanto el servicio esté disponible en tu calle.</strong></p>
                
                <div style="background-color: #F8FAF9; border-left: 4px solid #25815F; padding: 12px 15px; margin: 20px 0; border-radius: 0 8px 8px 0;">
                  <p style="margin: 0; font-size: 13px; color: #555;">
                    💡 <em>¿Conoces a vecinos o familiares en tu zona que también quieran el servicio? Cuantas más peticiones recibamos de tu área, antes abriremos la ruta.</em>
                  </p>
                </div>
            ...
            `,
            name: "TeLaTiro • Expansión de Rutas"
          });
        } catch (eCliZ) {
          console.warn("No se pudo enviar acuse a cliente zona:", eCliZ);
        }
      }

      return ContentService.createTextOutput(JSON.stringify({ "status": "success", "type": "new_zone" }))
        .setMimeType(ContentService.MimeType.JSON);
    }
    ...
    // ========================================================================
    var sheet = ss.getSheetByName("Solicitudes Clientes") || ss.getSheetByName("Solicitudes") || ss.getSheetByName("Clientes") || ss.getSheetByName("Hoja 1") || ss.getSheets()[0];
    if ((sheet.getName() === "Hoja de Ruta" || sheet.getName() === "Candidaturas" || sheet.getName() === "Colaboradores" || sheet.getName() === "Lista de Espera") && ss.getSheets().length > 1) {
      sheet = ss.getSheetByName("Solicitudes Clientes") || ss.getSheetByName("Solicitudes") || ss.getSheets()[0];
    }
...
function generarRutaDiaria(turnoManual) {
  ...
  // 🧭 ORDENAR PARADAS: TURNO MAÑANA PRIMERO, TURNO TARDE DESPUÉS (OPTIMIZADOS POR CERCANÍA)
  var paradas = [];
  var turnoNormalizado = (turnoSeleccionado + "").toLowerCase();
  
  if (turnoNormalizado.indexOf("todos") !== -1 || turnoNormalizado === "") {
    var paradasManana = [];
    var paradasTarde = [];
