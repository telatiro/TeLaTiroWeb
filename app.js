...
function getFormData() {
  ...
  let door = document.getElementById('clientDoor')?.value.trim() || '';
  ...
  if (!/\d+/.test(cleanStreetName) && door) {
    ...
    const matchCalleNum = cleanStreetName.match(/\b(\d+)\b/);
    if (matchCalleNum) {
      const nCalle = matchCalleNum[1];
      const doorSinPref = door.replace(/^(?:n[º°ª\.\/\-]?\s*|n[uú]mero\s*|num\.\s*|n\s*)/i, '');
      if (doorSinPref.trim() === nCalle) {
        door = '';
      } else {
        const escapeRegex = nCalle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const pat = new RegExp('^(?:n[º°ª\\.\\/\\-]?\\s*|n[uú]mero\\s*|num\\.\\s*|n\\s*)?' + escapeRegex + '\\s*[,\\-\\/\\.]+\\s*', 'i');
        door = door.replace(pat, '').trim();
      }
    }
  }

  door = door.replace(/^[,\-\/\.\s]+/, '').replace(/[,\-\/\.\s]+$/, '').trim();
